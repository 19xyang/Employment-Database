...
class Basicinformation(Table):
    ...
    view = LinkCol('View', 'view_employee', url_kwargs=dict(eid='eid'))
...
def edit_information(eid):
    ...
    try:
        cmd = 'UPDATE employee_have_job SET name=%s, class_year=%s, major=%s, university=%s,education_level=%s, linkedin=%s,jid=%s,industry=%s,job_title=%s,type=%s,company_name=%s WHERE eid=%s and jid=%s;'
        g.conn.execute(cmd, (name,class_year,major,university,education_level,linkedin, jid, industry, job_title, type, company_name,eid, jid))
    except:
...
def delete_information(eid):
    ...
    jid = str(request.form['jid'])
    #cmd="DELETE FROM employee_have_job WHERE eid = %s;"
    try:
        g.conn.execute("DELETE FROM employee_have_job WHERE eid = %s and jid=%s;", (eid,jid,))
    except:
        flash('Information cannot be deleted!')
        return redirect("/")
    return redirect("/")
##################################################################################
# create table for search_employee
##################################################################################
class employeesearch(Table):
    eid = Col('eid')
    name = Col('name')
    class_year = Col('class_year')
    major = Col('major')
...
# view employee
##################################################################################
@app.route('/view_employee/<int:eid>', methods=['GET', 'POST'])
def view_employee(eid):
	if request.method == 'POST':
		company_name = str(request.form['company_name'])
		cursor = g.conn.execute("SELECT * FROM employee_have_job WHERE company_name = %s;", (company_name,))
		results = []
		for result in cursor:
			results.append({'eid':result['eid'],
                'name':result['name'],
                ...
                'company_name':result['company_name']})
		cursor.close()
		table = employeesearch(results)
		table.border = True
		if not results:
			table = "No employee found!"
		return render_template('view_employee.html', eid=eid, table=table)
	text = "No employee for this company"
	return render_template("view_employee.html", eid=eid, table=text)
...
class reviewresult(Table):
    ...
    view = LinkCol('View', 'view_review', url_kwargs=dict(eid='eid'))
...
def add_review():
    ...
    try:
        ...
        g.conn.execute(text(cmd),eid1=session['eid'], jid1=jid, rid1=currid, content1=content, rating1=rating)
    except:
...
def edit_wishlist(eid):
    ...
    jid1 = request.form['jid_pre'].rstrip()
    jid2 = request.form['jid_cur'].rstrip()
    ...
    try:
        g.conn.execute("UPDATE wishlist SET jid=%s"
                     "WHERE eid=%s and jid=%s;", (jid2 ,eid, jid1))
    except:
...
##################################################################################
# create table for search_review
##################################################################################
class reviewsearch(Table):
    eid = Col('eid')
    jid = Col('jid')
    rid = Col('rid')
    content = Col('content')
...
# view review
##################################################################################
@app.route('/view_review/<int:eid>', methods=['GET', 'POST'])
def view_review(eid):
	if request.method == 'POST':
		results = []
		jid = str(request.form['jid'])
		cursor = g.conn.execute("SELECT * FROM review WHERE jid = %s;", (jid,))
		results = []
		for result in cursor:
			results.append({'eid':result['eid'],'jid':result['jid'],'rid':result['rid'],'content':result['content'],'rating':result['rating']})
		cursor.close()
		table = reviewresult(results)
		table.border = True
		if not results:
			table = "No review found"
		return render_template('view_review.html', eid=eid, table=table)
	text = "No review for this job"
	return render_template("view_review.html", eid=eid, table=text)
