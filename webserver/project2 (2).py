...
def view_trackingaccount(aid):
	if request.method == 'POST':
        results = []
    	jid = request.form['jid']
    	cursor = g.conn.execute("SELECT * FROM review WHERE jid = %s ORDER BY rid;", (jid,))
    	results = []
    	for result in cursor:
        	results.append({'eid':result['eid'],
        					'jid':result['jid'],
    	...
    	table.border = True
    	if not results:
            table = "No review for {{jid}}"
    	return render_template('view_review.html', table=table)
    text = "No review for this job"
    return render_template("view_review.html", table=text)
